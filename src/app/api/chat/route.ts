...
import { systemPromptDoctor } from '@/config/instructions-doctor';
...
const vectorStoreDoctorIds = process.env.VECTOR_STORE_DOCTOR
  ? process.env.VECTOR_STORE_DOCTOR.split(',').map(id => id.trim()).filter(id => id)
  : [];
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { input, previous_response_id, mode = 'strains' } = body;
    ...
    if (!input) {
      ...
      return new Response(JSON.stringify({ 
        ...
        receivedBody: body,
        expectedFormat: { input: "string", previous_response_id: "string (optional)", mode: "string (optional)" }
      }), { 
        status: 400,
        headers: {
    ...
    // Select vector store and instructions based on mode
    const isDoctorMode = mode === 'doctors';
    const selectedVectorStoreIds = isDoctorMode ? vectorStoreDoctorIds : vectorStoreIds;
    const selectedInstructions = isDoctorMode ? systemPromptDoctor : systemPrompt;

    // Construct tools only if vector store IDs are available
    const tools = selectedVectorStoreIds.length > 0
      ? [{
          type: "file_search",
