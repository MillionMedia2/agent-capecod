...
import { FiSend, FiPlusCircle, FiDownload, FiClock, FiRotateCw, FiLoader, FiAlertCircle, FiMessageSquare, FiTrash2, FiChevronDown } from 'react-icons/fi';
...
export default function ChatInterface() {
  ...
  const [mode, setMode] = useState<'strains' | 'doctors'>('strains');
  ...
  const sendMessageWithRetry = async (messageContent: string, retryCount = 0): Promise<Response> => {
    ...
    try {
      const cacheKey = `${messageContent}-${currentResponseId}-${mode}`;
  ...
  }, [currentResponseId, isLoading, mode]);
  ...
  const handleModeChange = (newMode: 'strains' | 'doctors') => {
      setMode(newMode);
      setShowModeDropdown(false);
      ...
      {messages.length <= 1 && (
        <div className={styles.faqSection}>
          ...
          <div className="space-y-2">
            <button onClick={() => handleExampleQuestionClick("I want to have better focus")} className={styles.faqButton}>
            I want to have better focus
            </button>
            <button onClick={() => handleExampleQuestionClick("Can you suggest something fruity?")} className={styles.faqButton}>
            Can you suggest something fruity?
            </button>
            <button onClick={() => handleExampleQuestionClick("What is the best strain for chronic pain?")} className={styles.faqButton}>
            What is the best strain for chronic pain?
            </button>
            <button onClick={() => handleExampleQuestionClick("I have trouble sleeping")} className={styles.faqButton}>
